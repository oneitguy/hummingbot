...
class BitoreumAPIError(IOError):
...
def convert_from_exchange_trading_pair(ex_trading_pair: str) -> Optional[str]:
    ...
    if regex_match is None:
        return None
    # Bitoreum.io uses lowercase (btcusdt)
...
class BitoreumConfigMap(BaseConnectorConfigMap):
    connector: str = Field(default="bitoreum", client_data=None)
    bitoreum_api_key: SecretStr = Field(
        default=...,
    ...
    bitoreum_secret_key: SecretStr = Field(
        default=...,
        client_data=ClientFieldData(
...
KEYS = BitoreumConfigMap.construct()
