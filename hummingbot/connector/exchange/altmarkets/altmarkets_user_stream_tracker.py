...
from hummingbot.connector.exchange.bitoreum.bitoreum_api_user_stream_data_source import \
...
class BitoreumUserStreamTracker(UserStreamTracker):
    ...
    def __init__(self,
                 ...
                 bitoreum_auth: Optional[BitoreumAuth] = None,
                 trading_pairs: Optional[List[str]] = None):
        self._bitoreum_auth: BitoreumAuth = bitoreum_auth
        self._trading_pairs: List[str] = trading_pairs or []
        self._throttler = throttler or AsyncThrottler(Constants.RATE_LIMITS)
        super().__init__(data_source=BitoreumAPIUserStreamDataSource(
    ...
    def data_source(self) -> UserStreamTrackerDataSource:
        ...
        *required
        ...
        if not self._data_source:
            self._data_source = BitoreumAPIUserStreamDataSource(
                throttler=self._throttler,
                bitoreum_auth=self._bitoreum_auth,
                trading_pairs=self._trading_pairs
            )
