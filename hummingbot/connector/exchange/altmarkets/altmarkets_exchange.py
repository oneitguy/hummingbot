...
import hummingbot.connector.exchange.bitoreum.bitoreum_http_utils as http_utils
from hummingbot.connector.exchange.bitoreum.bitoreum_api_order_book_data_source import (
    BitoreumAPIOrderBookDataSource,
)
from hummingbot.connector.exchange.bitoreum.bitoreum_auth import BitoreumAuth
from hummingbot.connector.exchange.bitoreum.bitoreum_constants import Constants
from hummingbot.connector.exchange.bitoreum.bitoreum_in_flight_order import BitoreumInFlightOrder
from hummingbot.connector.exchange.bitoreum.bitoreum_order_book_tracker import BitoreumOrderBookTracker
from hummingbot.connector.exchange.bitoreum.bitoreum_user_stream_tracker import BitoreumUserStreamTracker
from hummingbot.connector.exchange.bitoreum.bitoreum_utils import (
    BitoreumAPIError,
...
class BitoreumExchange(ExchangeBase):
    """
    BitoreumExchange connects with Bitoreum.io exchange and provides order book pricing, user account tracking and
    ...
    def __init__(self,
                 ...
                 bitoreum_api_key: str,
                 bitoreum_secret_key: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True
                 ):
        """
        :param bitoreum_api_key: The API key to connect to private Bitoreum.io APIs.
        :param bitoreum_secret_key: The API secret.
        ...
        self._bitoreum_auth = BitoreumAuth(bitoreum_api_key, bitoreum_secret_key)
        self._set_order_book_tracker(BitoreumOrderBookTracker(
            throttler=self._throttler,
            trading_pairs=trading_pairs))
        self._user_stream_tracker = BitoreumUserStreamTracker(
            throttler=self._throttler,
            bitoreum_auth=self._bitoreum_auth,
            trading_pairs=trading_pairs)
        self._ev_loop = asyncio.get_event_loop()
        self._shared_client = None
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, BitoreumInFlightOrder]
    ...
    @property
    def in_flight_orders(self) -> Dict[str, BitoreumInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: BitoreumInFlightOrder.from_json(value)
    ...
    async def _api_request(self,
        ...
        parsed_response = await http_utils.api_call_with_retries(
            method=method,
            endpoint=endpoint,
            auth_headers=self._bitoreum_auth.get_headers if is_auth_required else None,
            params=params,
        ...
        if "errors" in parsed_response or "error" in parsed_response:
            ...
            raise BitoreumAPIError(parsed_response)
    ...
    async def _create_order(self,
        ...
        try:
            ...
            if isinstance(e, BitoreumAPIError):
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = BitoreumInFlightOrder(
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> CancellationResult:
        ...
        :param trading_pair: The market trading pair (Unused during cancel on Bitoreum.io)
    ...
    def stop_tracking_order_exceed_not_found_limit(self, tracked_order: BitoreumInFlightOrder):
        """
    ...
    async def _update_order_status(self):
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            for response, tracked_order in zip(responses, tracked_orders):
                if isinstance(response, BitoreumAPIError):
    ...
    async def _trigger_order_fill(self,
                                  tracked_order: BitoreumInFlightOrder,
    ...
    async def _user_stream_event_listener(self):
        ...
        BitoreumAPIUserStreamDataSource.
        """
    ...
    async def all_trading_pairs(self) -> List[str]:
        ...
        return await BitoreumAPIOrderBookDataSource.fetch_trading_pairs()

    async def get_last_traded_prices(self, trading_pairs: List[str]) -> Dict[str, float]:
        # This method should be removed and instead we should implement _get_last_traded_price
        return await BitoreumAPIOrderBookDataSource.get_last_traded_prices(
            trading_pairs=trading_pairs,
            throttler=self._throttler
        )
