...
from .bitoreum_active_order_tracker import BitoreumActiveOrderTracker
from .bitoreum_constants import Constants
from .bitoreum_order_book import BitoreumOrderBook
from .bitoreum_utils import BitoreumAPIError, convert_from_exchange_trading_pair, convert_to_exchange_trading_pair
from .bitoreum_websocket import BitoreumWebsocket


class BitoreumAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_new_order_book(self, trading_pair: str) -> OrderBook:
        ...
        snapshot_msg: OrderBookMessage = BitoreumOrderBook.snapshot_message_from_exchange(
            snapshot,
            snapshot_timestamp,
            metadata={"trading_pair": trading_pair})
        order_book = self.order_book_create_function()
        active_order_tracker: BitoreumActiveOrderTracker = BitoreumActiveOrderTracker()
    ...
    async def listen_for_trades(self, ev_loop: asyncio.BaseEventLoop, output: asyncio.Queue):
        ...
        while True:
            try:
                ws = BitoreumWebsocket(throttler=self._throttler)
                    ...
                    if response is not None:
                        for msg_key in list(response.keys()):
                            ...
                            if len(split_key) != 2:
                                ...
                                self.logger().info(f"Unrecognized message received from Bitoreum websocket: {response}")
                                continue
                            ...
                            for trade in response[msg_key]["trades"]:
                                ...
                                trade_msg: OrderBookMessage = BitoreumOrderBook.trade_message_from_exchange(
                                    trade,
                                    trade_timestamp,
                                    metadata={"trading_pair": trading_pair})
                    ...
                    if response is not None:
                        for msg_key in list(response.keys()):
                            ...
                            if Constants.WS_METHODS['ORDERS_UPDATE'] in msg_key:
                                order_book_msg_cls = BitoreumOrderBook.diff_message_from_exchange
                                split_key = msg_key.split(Constants.WS_METHODS['ORDERS_UPDATE'], 1)
                            elif Constants.WS_METHODS['ORDERS_SNAPSHOT'] in msg_key:
                                order_book_msg_cls = BitoreumOrderBook.snapshot_message_from_exchange
