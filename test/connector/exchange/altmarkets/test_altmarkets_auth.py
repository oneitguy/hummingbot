...
from hummingbot.connector.exchange.bitoreum.bitoreum_websocket import BitoreumWebsocket
from hummingbot.logger.struct_logger import METRICS_LOG_LEVEL
from hummingbot.connector.exchange.bitoreum.bitoreum_constants import Constants
from hummingbot.connector.exchange.bitoreum.bitoreum_utils import aiohttp_response_with_errors
...
class TestAuth(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        secret_key = conf.bitoreum_secret_key
        cls.auth = BitoreumAuth(api_key, secret_key)
    ...
    async def ws_auth(self) -> Dict[Any, Any]:
        ws = BitoreumWebsocket(self.auth)
        await ws.connect()
